export type DatePeriod = 'today' | 'week' | 'biweekly' | 'month' | 'semester' | 'year';
...
export function getDateRange(period: DatePeriod, weekStartsOnMonday = true): DateRange {
  const now = new Date();
  ...
  switch (period) {
    ...
    case 'week': {
      ...
      const diff = weekStartsOnMonday
        ? currentDay === 0 ? 6 : currentDay - 1  // Lunes = 0
